    ...
    protected function defineDatabaseMigrations(): void
    {
        ...
        if (!Schema::hasTable('life_log_raw_measurements')) {
            Schema::create('life_log_raw_measurements', function (Blueprint $table) {
                ...
                // Scale index for per-type range queries (backfill, T003)
                $table->index(['user_id', 'type', 'recorded_at']);
                $table->index('promoted_at');
                // NOTE: bucket_hour single-column index is DROPped by T003 migration.
                // Not present in the hand-built schema.
            });
        }
        ...
        // life_log_account_backfill_states
        if (!Schema::hasTable('life_log_account_backfill_states')) {
